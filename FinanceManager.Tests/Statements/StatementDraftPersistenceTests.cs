...
public sealed class StatementDraftPersistenceTests
{
    ...
    public async Task GetDraftAsync_ShouldReturnPersistedDraft()
    {
        ...
        var created = await sut.CreateDraftAsync(owner, "x.csv", Array.Empty<byte>(), CancellationToken.None);
    ...
    public async Task AddEntryAsync_ShouldAppendEntry()
    {
        ...
        var draft = await sut.CreateDraftAsync(owner, "y.csv", Array.Empty<byte>(), CancellationToken.None);
    ...
    public async Task CancelAsync_ShouldRemoveDraft()
    {
        var (sut, db) = Create();
        var owner = Guid.NewGuid();
        var draft = await sut.CreateDraftAsync(owner, "z.csv", Array.Empty<byte>(), CancellationToken.None);
    ...
    public async Task CommitAsync_ShouldPersistImportAndEntries_AndMarkDraftCommitted()
    {
        ...
        var draft = await sut.CreateDraftAsync(owner, "c.csv", Array.Empty<byte>(), CancellationToken.None);

...
public sealed class ContactServiceTests
{
    ...
    public async Task CreateAsync_ShouldCreateContact_WhenValid()
    {
        ...
        var dto = await sut.CreateAsync(owner, "Alice", ContactType.Person, null, CancellationToken.None);
    ...
    public async Task CreateAsync_ShouldThrow_WhenTypeSelf()
    {
        var (sut, _) = Create();
        var owner = Guid.NewGuid();
        Func<Task> act = () => sut.CreateAsync(owner, "Me", ContactType.Self, null, CancellationToken.None);
    ...
    public async Task UpdateAsync_ShouldUpdateNameAndType_WhenNotSelf()
    {
        var (sut, db) = Create();
        var owner = Guid.NewGuid();
        var created = await sut.CreateAsync(owner, "BankX", ContactType.Organization, null, CancellationToken.None);

        var updated = await sut.UpdateAsync(created.Id, owner, "BankY", ContactType.Bank, null, CancellationToken.None);
    ...
    public async Task UpdateAsync_ShouldThrow_WhenChangingFromSelf()
    {
        ...
        Func<Task> act = () => sut.UpdateAsync(self.Id, owner, "Me2", ContactType.Person, null, CancellationToken.None);
        await act.Should().ThrowAsync<ArgumentException>()
            .WithMessage("*Self*");
    }
    ...
    public async Task UpdateAsync_ShouldThrow_WhenChangingToSelf()
    {
        var (sut, db) = Create();
        var owner = Guid.NewGuid();
        var created = await sut.CreateAsync(owner, "Org", ContactType.Organization, null, CancellationToken.None);

        Func<Task> act = () => sut.UpdateAsync(created.Id, owner, "Org", ContactType.Self, null, CancellationToken.None);
        await act.Should().ThrowAsync<ArgumentException>()
            .WithMessage("*Self*");
    }
    ...
    public async Task DeleteAsync_ShouldDelete_WhenNotSelf()
    {
        var (sut, db) = Create();
        var owner = Guid.NewGuid();
        var c = await sut.CreateAsync(owner, "Temp", ContactType.Other, null, CancellationToken.None);

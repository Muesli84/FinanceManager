...
public sealed class PostingsSavingsPlanViewModelTests
{
    ...
    public async Task Initialize_LoadsFirstPage_SetsItemsAndFlags()
    {
        ...
        {
            if (req.Method == HttpMethod.Get && req.RequestUri!.AbsolutePath.StartsWith("/api/postings/savings-plan/"))
            {
                var json = PostingsJson(15);
                return new HttpResponseMessage(HttpStatusCode.OK) { Content = new StringContent(json, Encoding.UTF8, "application/json") };
            }
            return new HttpResponseMessage(HttpStatusCode.NotFound);
        });
        var vm = new PostingsSavingsPlanViewModel(CreateSp(), new TestHttpClientFactory(client));
    ...
    public async Task LoadMore_AppendsItems_StopsWhenBelowPageSize()
    {
        ...
        var client = CreateHttpClient(req =>
        {
            if (req.RequestUri!.AbsolutePath.StartsWith("/api/postings/savings-plan/") && req.Method == HttpMethod.Get)
            {
                call++;
                var cnt = call == 1 ? 50 : 1;
    ...
    {
        var vm = new PostingsSavingsPlanViewModel(CreateSp(), new TestHttpClientFactory(CreateHttpClient(_ => new HttpResponseMessage(HttpStatusCode.OK))));

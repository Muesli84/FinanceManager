...
| FA-AUSZ-009 | Wertpapierzuordnung bei eigener Bank                                    | UI & API: Auswahl, Neuanlage & Zuordnung; PDF-Detailimport (ING) erkennt Dividende/Kauf/Verkauf, ISIN, Nominale, Ordernr., Steuern, Provision; Security-Postings; Menge Dividende optional; Validierung angepasst; Depot-/Positionslogik offen | ~      |
...
| FA-AUSZ-015 | Massenbuchung Kontoauszüge (inkl. optionaler Einzelbuchung pro Eintrag) | Neuer Background-Task (BookingTaskExecutor + BackgroundTaskManager, BookingTaskExecutor), Endpoint `/api/statement-drafts/book-all`, UI Dialog „Einträge einzeln buchen“, Fortschritt über BackgroundTaskStatusPanel | ✔      |
...
| FA-BACK-004 | Backup wiederherstellen (asynchron) mit Fortschrittsanzeige             | Vereinheitlicht: BackgroundTaskManager + BackupRestoreTaskExecutor + BackgroundTasksController + UI BackgroundTaskStatusPanel                          | ✔      |
...
| FA-SPAR-006 | Statusanzeige Sparziel erreicht                                         | AnalyzeAsync + Hinweise in Validierung (Ziel erreicht / fällige Pläne)                                                                                 | ~      |
...
| NFA-REL-002 | Langläufer im Hintergrund mit Fortschritt (Restore)                     | Vereinheitlicht über BackgroundTaskManager + Executor (BackupRestoreTaskExecutor) + Panel                                                              | ✔      |
| NFA-REL-003 | Einheitliche Background-Task Queue (Classify, Mass Booking, Restore)    | BackgroundTaskManager + BackgroundTaskRunner + Executors (ClassificationTaskExecutor, BookingTaskExecutor, BackupRestoreTaskExecutor) + API + UI Panel | ✔      |
